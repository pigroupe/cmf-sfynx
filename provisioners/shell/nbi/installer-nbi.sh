...
if [ ! -d "rapp_nosbelidees" ]; then
 git clone https://github.com/RappFrance/rapp_nosbelidees rapp_nosbelidees
fi
...
sudo cat > /tmp/nosbelidees << 'EOF'
upstream php5-fpm-sock {  
    server unix:/var/run/php5-fpm.sock;  
}

server {
    set $website_root "/var/www/rapp_nosbelidees/web";
    set $default_env  "app_dev.php";

    listen 80;

    # Server name being used (exact name, wildcards or regular expression)
    server_name dev.nosbelidees.local;
    ...
    access_log off; 
    log_not_found off; 
    #error_log  /var/log/nginx/nosbelidees-error.log;
...
server {
    ...
    # Server name being used (exact name, wildcards or regular expression)
    server_name test.nosbelidees.local;
...
server {
    ...
    # Server name being used (exact name, wildcards or regular expression)
    server_name prod.nosbelidees.local;
...
EOF
sudo mv /tmp/sfynx /etc/nginx/sites-available/nosbelidees

# we create the symbilic link
sudo ln -s /etc/nginx/sites-available/nosbelidees /etc/nginx/sites-enabled/nosbelidees

# we add host in the /etc/hosts file
if ! grep -q "dev.nosbelidees.local" /etc/hosts; then
    echo "Adding QA hostname to your /etc/hosts"
    echo "127.0.0.1    dev.nosbelidees.local" | sudo tee --append /etc/hosts
    echo "127.0.0.1    test.nosbelidees.local" | sudo tee --append /etc/hosts
    echo "127.0.0.1    prod.nosbelidees.local" | sudo tee --append /etc/hosts
fi

#
sudo chown -R www-data:www-data /var/www/rapp_nosbelidees
