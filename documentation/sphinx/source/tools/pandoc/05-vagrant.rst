...
If you have problem of ^M caracter in shell scripts, execute
/bin/dos2unix from all vagrant provisioning files like this

sous linux : find . -name '*.sh' \|xargs dos2unix sous windows : for /R
%G in (*.sh) do dos2unix "%G" find /R %G in
...
**Resolve** \* rm //.vagrant.d/data/lock.fpcollision.lock \* rm
/home//.vagrant.d/data/lock.fpcollision.lock \* find /home//.vagrant.d
-exec ls -al {} ; \* rm -rf /home//.vagrant.d

SSH
===

vagrant plugin install vagrant-vbguest sometime error like this Running
provisioner: file... Failed to upload a file to the guest VM via SCP due
to a permissions error. This is normally because the SSH user doesn't
have permission to write to the destination location. Alternately, the
user running Vagrant on the host machine may not have permission to read
the file. solution>>>> vagrant ssh => sudo chmod -R 777 /tmp => exit
