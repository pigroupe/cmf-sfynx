...
use Sfynx\CmfBundle\Builder\PiFormBuilderManagerInterface;
...
class PiFormBuilderManager extends AbstractType implements PiFormBuilderManagerInterface
{
    ...
    private function isServiceSupported($container, $actionName)
    {
        if (!isset($GLOBALS['FORM'][strtoupper($container)][strtolower($actionName)])) {
            throw ServciceException::serviceGlobaleUndefined(strtolower($actionName), 'FORM');
        } elseif (!$this->container->has($GLOBALS['FORM'][strtoupper($container)][strtolower($actionName)])) {
            throw ServciceException::serviceNotSupported($GLOBALS['FORM'][strtoupper($container)][strtolower($actionName)]);
        }
    ...
    {
        if ($this->getTypeForm() == "zend") {
            $foundBundle  = $this->container->get('kernel')->getBundle('SfynxTemplateBundle');
    ...
    private function renderForm()
    {
        if ($this->getTypeForm() == "zend") {
            ...
            $response = $this->container->get('templating')->renderResponse($this->container->getParameter('sfynx.auth.theme.layout.admin.form.builder').":{$this->_form_template}", array(
...
//         $this->_form->addElement($instanceElement);    
    ...
    private function setControlAction()
    {
        ...
        if ( ($this->isSubmitted()) ) {
            ...
            $this->actionSubmit();    
            // Redirection
            $new_url     = $this->container->get('sfynx.tool.route.factory')->getRoute();
     ...
     * @since 2012-09-11
     */
    private function setFlash($message, $type = "notice")
    {
    ...
    {
        try {
            $entity  = new \Sfynx\CmfBundle\Entity\Widget();
            $block   = $this->_em->getRepository('SfynxCmfBundle:Block')->find($this->_id_block);            
            if ($block instanceof \Sfynx\CmfBundle\Entity\BLock) {
    ...
    {
        $widget   = $this->_em->getRepository('SfynxCmfBundle:Widget')->find($this->_id_widget);        
        try {
