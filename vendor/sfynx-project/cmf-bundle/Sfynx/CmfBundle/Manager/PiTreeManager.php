...
use Sfynx\CmfBundle\Builder\PiTreeManagerBuilderInterface;
...
{    
    ...
    public function defaultNavigation($locale, $entity, $category, $template, $separatorClass = "", $ulClass = "", $liClass = "", $params = null)
    {
        ...
        try {
            ...
            if (isset($params['query_function']) && !empty($params['query_function'])) {
            ...
            if (isset($params['searchFields']) && !empty($params['searchFields'])) {
            	if (
                	count($params['searchFields']) == 2
                	&&
                	isset($params['searchFields']['nameField'])
            	) {
            		if (!empty($params['searchFields']['nameField'])) {
            			$query->andWhere('node.'.$params['searchFields']['nameField'] .' :value')
            			->setParameters(array(
            					'value'   => $params['searchFields']['valueField']
            			));
            		}
            	} else {
            		foreach ($params['searchFields'] as $searchFields) {
            ...
            if (!empty($template)) {
                $nodes = $em->getRepository($entity)->findTranslationsByQuery($locale, $query->getQuery(), 'object', false); 
            } else {
        ...
        if (!empty($template)) {
            ...
            $response             = $this->container->get('templating')->renderResponse("SfynxTemplateBundle:Template\\Tree:$template", $params);
            ...
            $options = array(
                    ...
                    'nodeDecorator' => function($node) use (&$self) {
                        ...
                        if ( method_exists($menu, 'getPage') && ($menu->getPage() InstanceOf \Sfynx\CmfBundle\Entity\Page) ) {
                            $routename     = $menu->getPage()->getRouteName();
                            $url        = $self->getContainer()->get('sfynx.tool.route.factory')->getRoute($menu->getPage()->getRouteName(), array('locale'=>$self->locale));
    ...
    public function defaultOrganigram($locale, $entity, $category, $params = null)
    {
        ...
        if (!empty($template)) {
        	...
        	$params['nodes']      = $nodes;
        	$params['repository'] = $em->getRepository($entity);
        	$response             = $this->container->get('templating')->renderResponse("SfynxTemplateBundle:Template\\Organigram:$template", $params);
        	$tree                 = $response->getContent() . " \n";
        	$tree                 = $this->container->get('sfynx.tool.string_manager')->closetags($tree);
        	$tree                 = utf8_decode(mb_convert_encoding($tree, "UTF-8", "HTML-ENTITIES"));
        } else {
            ...
            $options = array(
                    ...
                    'nodeDecorator' => function($node) use (&$self) {       
                        ...
                        if ( method_exists($tree, 'getPage') && ($tree->getPage() InstanceOf \Sfynx\CmfBundle\Entity\Page) ) {
                            $routename  = $tree->getPage()->getRouteName();
                            $url        = $self->getContainer()->get('sfynx.tool.route.factory')->getRoute($tree->getPage()->getRouteName(), array('locale'=>$self->locale));
                        } else {
                            $routename  = '';
                            $url        = "#"; //$self->getContainer()->get('router')->generate($routename);
                        }               
