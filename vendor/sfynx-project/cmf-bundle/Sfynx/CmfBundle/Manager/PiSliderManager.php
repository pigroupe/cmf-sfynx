    ...
    public function getSlider($locale, $controller, $category, $template, $parameters = null)
    {
        ...
        foreach ($allslides as $key => $slide) {
            if (method_exists($slide, 'getPosition')) {
                ...
                if (method_exists($slide, 'getPage') && ($slide->getPage() instanceof \Sfynx\CmfBundle\Entity\Page) ) {
            ...
            $templateContent = $this->container->get('twig')->loadTemplate("SfynxTemplateBundle:Template\\Slider:$template");    
