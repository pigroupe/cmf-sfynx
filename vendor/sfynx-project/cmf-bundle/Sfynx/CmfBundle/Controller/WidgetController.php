...
class WidgetController extends CmfabstractController
{
    ...
    public function showAction($id)
    {
        ...
        return $this->render('SfynxCmfBundle:Widget:show.html.twig', array(
            'entity'      => $entity,
    ...
    public function editAction($id)
    {
        ...
        return $this->render("SfynxCmfBundle:Widget:$template", array(
            'entity'      => $entity,
    ...
    public function moveajaxAction()
    {
        ...
        if ($request->isXmlHttpRequest()){
            ...
            if (!is_null($id) && !is_null($type) && in_array($type, array('up', 'down')) ){
                ...
                if ($type == 'up'){
                    $all_widget_block     = $em->getRepository('SfynxCmfBundle:Widget')->getAllWidgetsByBlock($entity_block->getId(), "DESC")->getResult(); //$entity_block->getWidgets();
                    foreach($all_widget_block as $key => $widget){
                ...
                elseif ($type == 'down'){
                    $all_widget_block     = $em->getRepository('SfynxCmfBundle:Widget')->getAllWidgetsByBlock($entity_block->getId(), "ASC")->getResult(); //$entity_block->getWidgets();
                    foreach($all_widget_block as $key => $widget){
                        $widg_pos = $widget->getPosition();
                        if (!$break && ($widg_pos > $entity_widget_pos) ){
                            $widget->setPosition($entity_widget_pos);
