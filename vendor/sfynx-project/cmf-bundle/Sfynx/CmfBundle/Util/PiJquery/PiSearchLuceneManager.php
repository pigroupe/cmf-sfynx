...
use Sfynx\CmfBundle\Manager\PiLuceneManager;
...
class PiSearchLuceneManager extends PiJqueryExtension
{
    ...
    {
        // css
        //$this->container->get('sfynx.tool.twig.extension.layouthead')->addCssFile('bundles/sfynxtemplate/css/themes/rocket/jquery_ui.css');
        //$this->container->get('sfynx.tool.twig.extension.layouthead')->addCssFile("bundles/sfynxtemplate/css/themes/wijmo/jquery.wijmo-open.2.1.2.css");
        
        // wijmo
        $this->container->get('sfynx.tool.twig.extension.layouthead')->addJsFile("bundles/sfynxtemplate/js/wijmo/external/jquery.wijmo-open.all.2.1.2.min.js");
    }    
    ...
    private function renderpageAction($archive, $options = null)
    {
        ...
        if (is_array($archive)){            
            ...
            foreach($archive as $key => $tags){
                ...
                $year          = $this->container->get('sfynx.tool.date_manager')->format($datetime, 'long', 'medium', $this->locale.'_'.$this->locale, 'Y');
                $Timestamp     = $datetime->getTimestamp();
                
                $pagination[$year][$Timestamp][] =  $tags;
            }            
        }
        $response    = $this->container->get('templating')->renderResponse("SfynxTemplateBundle:Template\\Search:$template", array(
                'archive'      => $archive,
