...
class HandlerLogin
{
    ...
    public function onKernelResponse(FilterResponseEvent $event)
    {
        ...
        $this->container->get('event_dispatcher')->dispatch(BootStrapUserEvents::HANDLER_LOGIN_CHANGEREDIRECTION, $event_redirection);
        ...
        $this->container->get('event_dispatcher')->dispatch(BootStrapUserEvents::HANDLER_LOGIN_CHANGERESPONSE, $event_response);
    ...
    protected function setParams()
    {
        ...
        if (!empty($BEST_ROLE_NAME)) {
            ...
            if ($role instanceof \BootStrap\UserBundle\Entity\Role) {
                ...
                if ($role->getLayout() instanceof \BootStrap\UserBundle\Entity\Layout) {
    ...
    protected function setLayout()
    {
        ...
        if (!is_null($browser) && $browser->isMobileDevice) {
            ...
            if ( $this->security->isGranted('ROLE_CONTENT_MANAGER') || $this->security->isGranted('ROLE_ADMIN') || $this->security->isGranted('ROLE_SUPER_ADMIN') ) {
                $layout    = 'PiAppTemplateBundle::Template\\Layout\\Mobile\\Admin\\'. $SfynxScreen . '.html.twig';
            } elseif ( $this->security->isGranted('ROLE_USER') ) {
                $layout    = 'PiAppTemplateBundle::Template\\Layout\\Mobile\\User\\'. $SfynxScreen . '.html.twig';
            } else {
                $layout    = 'PiAppTemplateBundle::Template\\Layout\\Mobile\\Default\\'. $SfynxScreen . '.html.twig';
            }
        } else {
            //     we calculate the layout to be applied.
        ...
        $request->attributes->set('sfynx-layout', $layout);
        $this->layout =  $layout;
    }    
