                    ...
                    {
                        $('#grid_customer').dataTable().fnFilter( 
                            $("#global_filter").val(),
                            null, 
                        ...
                        $("table th").each( function ( i ) {
                                ...
                                if (column != undefined) {
                            				...
                            				$("table th").each( function ( i ) {
                                                var column = $(this).data('search');
                                                if (column == 8) {
                                                    $(this).html('<div id="filter-grid-date-1" ><input class="form-control form-control-inline input-medium default-date-picker" type="text" id="minp" name="minp"><input type="text" class="form-control form-control-inline input-medium default-date-picker" id="maxp" name="maxp"></div>');
                                                }
                            				}); 
                            				$("#maxp").datepicker({
                                                changeMonth: true,
                                                ...
                                                buttonImage: "/bundles/sfynxtemplate/images/icons/form/picto-calendar.png",
                                                onSelect: function(date) {
                                                	maxpDateFilter = new Date(date).getTime();
                                                    grid_customeroTable.fnDraw();
                                                    $(this).datepicker('hide');
                                                }
                                            }).keyup( function () {
                                            	maxpDateFilter = new Date(this.value).getTime();
                                                grid_customeroTable.fnDraw();
                                                $(this).datepicker('hide');
                                            }).on('changeDate', function(ev){
                                            	maxpDateFilter = new Date(ev.date).getTime();
                                                grid_customeroTable.fnDraw();
                                                $(this).datepicker('hide');
                                            });
                            				$("#minp").datepicker({
                                                ...
                                                buttonImage: "/bundles/sfynxtemplate/images/icons/form/picto-calendar.png",
                                                onSelect: function(date) {
                                                	minpDateFilter = new Date(date).getTime();
                                                    grid_customeroTable.fnDraw();
                                                    $(this).datepicker('hide');
                                                  }
                                            }).keyup( function () {
                                            	minpDateFilter = new Date(this.value).getTime();
                                                grid_customeroTable.fnDraw();
                                                $(this).datepicker('hide');
                                            }).on('changeDate', function(ev){
                                                <!-- http://bootstrap-datepicker.readthedocs.org/en/release/methods.html#setdate -->                                                
                            				...
                            				$.datepicker.setDefaults( $.datepicker.regional[ "en" ] );
                            			                            		                                    if (values == undefined) {
                                        values = grid_customeroTable.fnGetColumnData(column) 
                                    }
                                    if (type != "input") {
                                        var options = [];
                                        ...
                                        $('select', this).change( function () {
                                            ...
                                            grid_customeroTable.fnFilter( values, column, true );
                                        });
                                            ...
                                            filter: function(e, matches) {
                                                ...
                                                if ( (ajaxsearch === true) || (ajaxsearch === 'true') ) {
                                                	if(keyword != $(e.target).data('keyword')) {
                                                    	grid_customeroTable.fnFilter( keyword, column, true );
                                                	}
                                            	}
                                            }
                                        });                                        
                                        ...
                                        $(this).find('input').width('91%').attr('id', 'input_'+i).data('title', title).data('column', column).keyup( function () {
                                            ...
                                            search_timeout = setTimeout(function() {
                                              ...
                                              grid_customeroTable.fnFilter( $this.value, column, true );
                                            }, 1000);
                                        } );
                                    }
...
/*
                        $("[id^='select_']").change( function () {
                            ...
                            grid_customeroTable.fnFilter( values, $(this).data('column'), true );
                        });
                        $("[id^='select_']").multiselect({
                            ...
                            filter: function(e, matches) {
                                ...
                                var keyword = $('.ui-multiselect-filter:visible').find('input').val();
                                if(keyword != $(e.target).data('keyword')) {
                                    grid_customeroTable.fnFilter( keyword, $(this).data('column'), true );
                                }
                            }
                        });
                        var foo_input = function() {
                            grid_customeroTable.fnFilter( $(this).val(), $(this).data('column'), true );
                        };
...
*/
                    ...
                    $('#grid_customer tbody tr').each(function(index) {
                        $(this).find("td.position").prependTo(this);
                    });
                    $('#grid_customer thead tr').each(function(index) {
                    ...
                    $("tfooter input").keyup( function () {
                        grid_customeroTable.fnFilter( this.value, grid_customeroTable.oApi._fnVisibleToColumnIndex( 
                                grid_customeroTable.fnSettings(), $("thead input").index(this) ) );
                    } );
					*/
                    
                                                                                                                                                        // Set up enabled row
                            enabled = new $.fn.dataTable.Editor( {
                                "domTable": "#grid_customer",
                                //"display": "envelope",
                                "ajaxUrl": "/admin/users/enabled?_token=fbd9b09d82e8bc8144812c85e09a01988b5c1bd6",
                                "events": {
                            ...
                            disablerow = new $.fn.dataTable.Editor( {
                                "domTable": "#grid_customer",
                                //"display": "envelope",
                                "ajaxUrl": "/users/disable?_token=fbd9b09d82e8bc8144812c85e09a01988b5c1bd6",
                                "events": {
                                    "onPreSubmit": function (data) {
                                    },
                                    "onPostSubmit": function (json, data) {
                            ...
                            archiverow = new $.fn.dataTable.Editor( {
                                "domTable": "#grid_customer",
                                //"display": "envelope",
                                "ajaxUrl": "/users/archive?_token=fbd9b09d82e8bc8144812c85e09a01988b5c1bd6"
                            } );                            
                                                                            

                    grid_customeroTable = $('#grid_customer').dataTable({
                        "bPaginate":true,
                        "bRetrieve":true,
                        "bFilter": true,
                        ...
                        'fnServerData' : function ( sSource, aoData, fnCallback ) {
    						    ...
    						    aoData.push( { 'name' : 'date-maxc', 'value' : $("#maxc").val() } );
    						        						    aoData.push( { 'name' : 'date-minp', 'value' : $("#minp").val() } );
    						    aoData.push( { 'name' : 'date-maxp', 'value' : $("#maxp").val() } );
                        ...
                        "oTableTools": {
                            ...
                            "aButtons": [
                                                ...
                                                "sExtends": "csv",
                                                "sTitle": "Sfynx",
                                                "sButtonText": "<img class='btn-action' src='/bundles/sfynxsmoothness/admin/grid/img/export_csv.png?v1_0_0' title='Export' alt='Export'  />CSV"
                                            },
                                                                                                ...
                                                                                                "formTitle": "Désactiver données",
                                                                                                  "question": function(b) {
                                                                                                      return "Voulez-vous désactiver " + b + " ligne" + (b === 1 ? " ?" : "s ?")
                                                                                                                                                        ...
                                                                                                                                                        {
                                                "sExtends": "editor_remove",
                                                "sButtonText": "<img class='btn-action' src='/bundles/sfynxsmoothness/admin/grid/img/archive.png?v1_0_0' title='Archive' alt='Archive'  />Archive",
                                                "editor": archiverow,
                                                "formButtons": [
                                                    {
                                                        "label": "Valider",
